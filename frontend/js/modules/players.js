...
import {
  ...
  findRosterPlayer,
  formatStatValue,
  getPlayerDisplayNameOverride,
  normalizeKey,
  normalizeString,
  parseStatNumber,
  playerProfileKey,
  POSITION_MAP_LAYOUT
} from './players/player-data.js';
import { createPlayerProfileView } from './players/player-profile-view.js';
import { createTeamProfileView } from './players/team-profile-view.js';
...
const renderTeamProfile = (playersGrid) => {
  ...
  const teamProfile = createTeamProfileView({
    team,
    ...
    fixtureVisibleCount: state.fixtureVisibleCount || 10,
    standings: state.standings,
    leagueTeams: state.leagueTeams,
    isFollowed: isTeamFollowed(team.id),
    buildFollowButton,
    helpers: {
      createTextElement,
      getFlagUrl,
      ...
      normalizeKey,
      toDateSafe,
      competitionLogos: COMPETITION_LOGOS,
      getLeagueConfig,
      trophyImages: TROPHY_IMAGES,
      vsDesignUrl: VS_DESIGN_URL,
      splitTeamName,
      buildLogo,
      positionGroupFor,
...
const renderPlayerProfile = (playersGrid) => {
  ...
  const wrapper = createPlayerProfileView({
    profile,
    isFollowed: isPlayerFollowed(profile.key),
    compareState: {
...
const openPlayerProfileInTeam = (playerName, playersGrid) => {
  ...
  state.activePlayerProfile = createPlayerProfileData({
    player,
    team,
    leagueKey: state.activeLeague,
    teams: state.teams,
...
export const initPlayers = () => {
  ...
  playersGrid.addEventListener('click', async (event) => {
    ...
    if (playerCompareBtn && state.activePlayerProfile) {
      ...
      renderPlayerProfile(playersGrid);
      if (state.compareOpen) focusPlayerCompareInput(playersGrid);
      return;
    }
